...
async function moderateOutboundMessage(openAIClient, message, fallbackMessage) {
  ...
  if (moderation.flagged) {
    console.warn(`Message flagged by OpenAI moderation API: ${JSON.stringify(moderation)}`);
    return {
      message: fallbackMessage,
      flagged: true,
    };
...
async function moderateInboundReply(openAIClient, reply, fallbackMessage) {
  ...
  if (moderation.flagged) {
    console.warn(`Reply flagged by OpenAI moderation API: ${JSON.stringify(moderation)}`);
    return {
      reply: fallbackMessage,
      flagged: true,
    };
