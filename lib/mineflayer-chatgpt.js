...
const DEFAULT_MINIMUM_REPLY_CONFIDENCE_SCORE = 0.9;
const DEFAULT_MINIMUM_JAILBREAK_CONFIDENCE_SCORE = 0.7;
...
function chatgpt(bot) {
  ...
  let minimumReplyConfidenceScore;
  let minimumJailbreakConfidenceScore;
   ...
   * @param {string} [opts.moderationApiKey] - OpenAI API key used for moderation and jailbreak detection. Required when `enableModeration` is `true`.
   ...
   * @param {number} [opts.minimumReplyConfidenceScore=0.9] - Minimum confidence required to accept a model reply.
   * @param {number} [opts.minimumJailbreakConfidenceScore=0.7] - Minimum OpenAI Guardrails confidence required to flag a jailbreak attempt.
   */
  bot.chatgpt.setConfig = (opts) => {
    opts = opts || {};
    enableModeration =
      opts.enableModeration === undefined
        ? DEFAULT_ENABLE_MODERATION
        : opts.enableModeration;
    validateEnableModeration(enableModeration);
    enableMessageLogging =
      opts.enableMessageLogging === undefined
        ? DEFAULT_ENABLE_MESSAGE_LOGGING
        : opts.enableMessageLogging;
    validateEnableMessageLogging(enableMessageLogging);
    minimumReplyConfidenceScore =
      opts.minimumReplyConfidenceScore ??
      DEFAULT_MINIMUM_REPLY_CONFIDENCE_SCORE;
    validateReplyConfidenceScore(minimumReplyConfidenceScore);
    minimumJailbreakConfidenceScore =
      opts.minimumJailbreakConfidenceScore ??
      DEFAULT_MINIMUM_JAILBREAK_CONFIDENCE_SCORE;
    validateJailbreakConfidenceScore(minimumJailbreakConfidenceScore);
    coolDownInSeconds =
      opts.coolDownInSeconds === undefined
        ? DEFAULT_COOL_DOWN_IN_SECONDS
        : opts.coolDownInSeconds;
    validateCoolDownInSeconds(coolDownInSeconds);
    fallbackMessage =
      opts.fallbackMessage === undefined
        ? DEFAULT_FALLBACK_MESSAGE
        : opts.fallbackMessage;
    validateFallbackMessage(fallbackMessage);
    opts.enableSecurityInstructions =
      opts.enableSecurityInstructions === undefined
        ? DEFAULT_ENABLE_SECURITY_INSTRUCTIONS
        : opts.enableSecurityInstructions;
    validateEnableSecurityInstructions(opts.enableSecurityInstructions);
